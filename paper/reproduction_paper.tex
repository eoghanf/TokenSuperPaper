...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../assets/front_page_figure.png}
    \caption{\textbf{Left:} Validation loss (BPB) during training for the standard
    40k-step baseline (dashed), the 20k-step Token Superposition run, and the
    20k-step schedule-matched baseline (standard AR with identical LR decay timing).
    The schedule-matched baseline closely tracks Token Superposition throughout.
    \textbf{Right:} Final validation BPB for each run. The gap between the
    schedule-matched baseline and Token Superposition (0.014 BPB, 15\% of TST's
    total advantage) is far smaller than the gap closed by schedule-matching alone.}
