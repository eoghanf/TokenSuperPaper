...
However, we argue that this comparison is confounded by the Warmup-Stable-Decay (WSD)
learning rate schedule.
Because TST trains for 20{,}000 steps while the baseline trains for 40{,}000, TST's LR
decay horizon arrives at half the total FLOPs, giving it a structural advantage
unrelated to bag training.
A schedule-matched baseline — standard autoregressive training for 20{,}000 steps with
an identical decay horizon — recovers 85\% of TST's apparent advantage with no
superposition at all.
The remaining 15\% attributable to bag training is real but modest (0.014~BPB),
and does not support the strong efficiency claims of the original paper.
\end{abstract}
